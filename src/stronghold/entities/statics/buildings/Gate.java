	...
	public void setLvUnder() {
		...
		if (isVertical) {
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)) + 1, ((int) (y / Tile.TILEHEIGHT)),
					1);
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)) + 1,
					((int) (y / Tile.TILEHEIGHT)) + 1, 2);
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)) + 1,
					((int) (y / Tile.TILEHEIGHT)) + 2, 1);
		} else {
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)), ((int) (y / Tile.TILEHEIGHT)) + 1,
					1);
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)) + 1,
					((int) (y / Tile.TILEHEIGHT)) + 1, 2);
			handler.getWorld().setGridNodeEntranceLv(((int) (x / Tile.TILEWIDTH)) + 2,
					((int) (y / Tile.TILEHEIGHT)) + 1, 1);
		}
			...
			for (int i = max; i < 2; i++)
				for (int j = 1; j < 5; j++) {
					g.drawImage(Assets.bars, (int) (x - handler.getGameCamera().getxOffset() + j * Tile.TILEWIDTH / 2),
							(int) (y - handler.getGameCamera().getyOffset() + i * Tile.TILEHEIGHT / 2),
							Tile.TILEHEIGHT / 2, Tile.TILEWIDTH / 2, null);
					g.drawImage(Assets.bars, (int) (x - handler.getGameCamera().getxOffset() + j * Tile.TILEWIDTH / 2),
							(int) (y - handler.getGameCamera().getyOffset() - (i + 1) * Tile.TILEHEIGHT / 2
									+ Tile.TILEHEIGHT * 3),
							Tile.TILEHEIGHT / 2, Tile.TILEWIDTH / 2, null);
				}
			...
			for (int i = max; i < 2; i++)
				for (int j = 1; j < 5; j++) {
					g.drawImage(Assets.bars, (int) (x - handler.getGameCamera().getxOffset() + i * Tile.TILEWIDTH / 2),
							(int) (y - handler.getGameCamera().getyOffset() + j * Tile.TILEHEIGHT / 2),
							Tile.TILEHEIGHT / 2, Tile.TILEWIDTH / 2, null);
					g.drawImage(Assets.bars,
							(int) (x - handler.getGameCamera().getxOffset() - (i + 1) * Tile.TILEWIDTH / 2
									+ Tile.TILEWIDTH * 3),
							(int) (y - handler.getGameCamera().getyOffset() + j * Tile.TILEHEIGHT / 2),
							Tile.TILEHEIGHT / 2, Tile.TILEWIDTH / 2, null);
				}

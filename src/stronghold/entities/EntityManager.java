...
public class EntityManager {
			...
			if (ticksToControlEnemies > TICKS_TO_CONTROL_ENEMIES) {
				if ((e instanceof Unit) && !((Unit) e).getIsPlayers()) {
					...
					if (path != null)
						...
						if (isBuildingClose(grid, (int) (e.getX() / Tile.TILEWIDTH), (int) (e.getY() / Tile.TILEHEIGHT))
								&& damageWall(e))
							;
						{
							...
							path = pathFinding.findPath(new Point((int) e.getX(), (int) e.getY()), new Point(
									nearestWall.getx() * Tile.TILEWIDTH, nearestWall.gety() * Tile.TILEHEIGHT));
							((Unit) e).setPath(path);
						}
					}
				}
	...
	private void MoveUnits(Point dest, List<Entity> unitsToMove) {
		...
		for (Entity unit : unitsToMove) {
			((Unit) unit)
					.setPath(pathFinding.findPath(new Point((int) unit.getX(), (int) unit.getY()), locations.get(i)));
	...
	public void damageUnits(int damage, Rectangle area, boolean isDmgFromPlayersUnit) {
		for (Entity e : entities) {
			if (!(e instanceof Unit))
				continue;
			if (((Unit) e).getIsPlayers() == isDmgFromPlayersUnit)
				continue;
	...
	public Unit enemyInRange(Unit attacker) {
		...
		for (Entity e : entities) {
			if (!(e instanceof Unit))
				continue;
			if (((Unit) e).getIsPlayers() == attacker.getIsPlayers())
				continue;
			...
			if (area.contains(e.getX() + e.getWidth() / 2, e.getY() + (e.getHeight() / 2))) {
				...
				if ((!(attacker instanceof Archer))
						&& (Math.abs(grid[eY][eX].getEntrenceLv() - grid[attackerX][attackerY].getEntrenceLv()) > 1))
					continue;
	...
	public void spawnEnemies(int x, int y, int difficulty) {
		List<Point> locations = pathFinding.findLocations(new Point(x * Tile.TILEWIDTH, y * Tile.TILEHEIGHT),
				difficulty * 6);
		if (locations == null)
			return;
		int i = 0;
